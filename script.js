const calculateTip = () => {
    ...
    // If user not provide correct input
     ...
     if(amount === '' || amount == 0 || amount < 0 || amount === "@" || amount ==='#' || amount ==='$'){
        document.getElementById("billEmpty").innerHTML = "Please Enter the Valid Bill Amount";
        return;
     }
     

    if(tipPercentage === '' || tipPercentage == 0 || tipPercentage < 0 || tipPercentage ==='@' || tipPercentage ==='#' || tipPercentage ==='$' ){
    ...
    if(numPeople === '' || numPeople == 0 || numPeople < 0 || numPeople ==='@' || numPeople ==='#' || numPeople ==='$'){
